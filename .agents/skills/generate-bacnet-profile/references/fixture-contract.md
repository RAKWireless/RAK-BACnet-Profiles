...
do not rename keys, turn source objects into strings, wrap payloads in objects,
or omit `name`, `fPort`, or string `input` from a test case:
...
Allowed source `type` values are `issue`, `official-document`,
`vendor-decoder`, and `customer-data`. The `profile` value is the Profile
